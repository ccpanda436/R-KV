...
def moba_attn_varlen(
    ...
    Supports both standard attention and GQA (Grouped Query Attention):
    - For standard attention: q, k, v have the same number of heads
    - For GQA: q has more heads than k/v, and attention scores are computed per group with max pooling

    Args:
        q (torch.Tensor): [seqlen, head, head_dim]
        k (torch.Tensor): [seqlen, kv_heads, head_dim] (kv_heads <= head for GQA)
        v (torch.Tensor): [seqlen, kv_heads, head_dim] (kv_heads <= head for GQA)
        ...
        moba_chunk_size (int): size of each chunk for moba attention
        moba_topk (int): number of top chunks to select for moba attention
    ...
    kv_heads = k.shape[1]
    query_group_size = num_head // kv_heads
    
    # key_gate_weight [ F_N_CHUNK, KV_HEADS, HEAD_DIM ]
    key_gate_weight = (
        filtered_kv[:, 0]
        .view(num_filtered_chunk, moba_chunk_size, kv_heads, head_dim)
        .mean(dim=1)
        .float()
    )
    ...
    if query_group_size == 1:
        # Standard attention - no grouping needed
        gate = torch.einsum(
            "nhd,shd->nhs", key_gate_weight, q
        )  # gate [ F_N_CHUNK, KV_HEADS, SEQ ]
    else:
        # GQA - group queries and apply pooling
        # Reshape q to [seqlen, kv_heads, query_group_size, head_dim]
        q_grouped = q.view(seqlen, kv_heads, query_group_size, head_dim)
        
        # Compute attention scores for each group
        # [F_N_CHUNK, kv_heads, query_group_size, seqlen]
        gate_grouped = torch.einsum(
            "nhd,shd->nhs", key_gate_weight, q_grouped
        )
        
        # Apply max pooling over query_group_size dimension
        gate = gate_grouped.max(dim=2).values  # [F_N_CHUNK, kv_heads, seqlen]
